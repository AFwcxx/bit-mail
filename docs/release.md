...
bit-mail v0.1.6 supports Rust 1.88 or newer on Linux and macOS. Rust 1.88 is
...
| Embedded runtime assets | binary version (`0.1.6`) |
...
gh attestation verify bit-mail-v0.1.6-<target>.tar.gz --repo AFwcxx/bit-mail
```

The workflow refuses a tag whose version differs from `Cargo.toml`. A pushed
`v0.1.6` tag builds the four archives and creates the GitHub Release with
