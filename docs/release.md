...
bit-mail v0.1.4 supports Rust 1.88 or newer on Linux and macOS. Rust 1.88 is
...
| Embedded runtime assets | binary version (`0.1.4`) |
...
gh attestation verify bit-mail-v0.1.4-<target>.tar.gz --repo AFwcxx/bit-mail
```

The workflow refuses a tag whose version differs from `Cargo.toml`. A pushed
`v0.1.4` tag builds the four archives and creates the GitHub Release with
...
references, runs the locked release test, creates a signed release commit and
tag, then atomically pushes both `main` and the tag. Run `./scripts/test-release.sh`
to exercise the workflow against a temporary local remote.
