...
bit-mail v0.1.8 supports Rust 1.88 or newer on Linux and macOS. Rust 1.88 is
...
| Embedded runtime assets | binary version (`0.1.8`) |
...
gh attestation verify bit-mail-v0.1.8-<target>.tar.gz --repo AFwcxx/bit-mail
```

The workflow refuses a tag whose version differs from `Cargo.toml`. A pushed
`v0.1.8` tag builds the four archives and creates the GitHub Release with
